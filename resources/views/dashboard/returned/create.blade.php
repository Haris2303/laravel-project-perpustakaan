    ...
    <div class="p-4 sm:ml-64">
        <div class="py-5 w-1/2 shadow-lg px-5 roudned-xl">
            @if (count($borrowings))
                ...
                <form action="/dashboard/returneds" method="post">
                    @csrf
                    <!-- Peminjaman -->
                    <div class="mt-4">
                        <x-input-label for="borrowing" :value="__('Pilih Peminjaman')" />
                        <select name="borrowing_id" id="borrowing"
            ...
            @else
                <h1 class="text-xl font-bold my-5">Tidak ada peminjaman buku</h1>
            @endif
        </div>
    </div>
</x-dashboard-layout>
