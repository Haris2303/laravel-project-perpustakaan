            ...
            <form action="/dashboard/book/{{ $book->isbn }}" method="post" enctype="multipart/form-data">
                            ...
                            <div>
                                <input type="checkbox" name="genres[]" id="{{ $genre->id }}"
                                    @foreach ($book->hasGenres as $item)
                                        {{ $genre->id === $item->id ? 'checked' : '' }} @endforeach
                                    value="{{ $genre->id }}"
                ...
                <div class="mt-4">
                    ...
                    <x-text-input id="publication_year" class="block mt-1 w-full" type="text" name="publication_year"
                        :value="old('publication_year', $book->publication_year)" required autofocus autocomplete="publication_year" />
                ...
                <div class="mt-4">
                    ...
                    <x-text-input id="publisher" class="block mt-1 w-full" type="text" name="publisher"
                        :value="old('publisher', $book->publisher)" required autofocus autocomplete="publisher" />
                ...
                <div class="mt-4">
                    ...
                    <textarea name="description" id="description"
                        ...
                        rows="5" required autocomplete="description">
                ...
                <div class="mt-4">
                    ...
                    <x-text-input id="shell_code" class="block mt-1 w-full" type="text" name="shell_code"
                        :value="old('shell_code', $book->shell_code)" required autofocus autocomplete="shell_code" placeholder="4 karakter" />
                    <x-input-error :messages="$errors->get('shell_code')" class="mt-2" />
                </div>

                <button type="submit" class="w-full py-2 rounded text-white mt-5 bg-slate-700">Ubah</button>
            </form>
        </div>
    </div>
</x-dashboard-layout>
